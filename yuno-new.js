...
// Mobile detection utility (simple check)
...
// Template with enhanced mobile-friendly design
...
```
  ...
  /* Enhanced trigger pill with micro-animations */
  .bubble {
    display: inline-flex;
    align-items: center;
  ...
  /* Enhanced teaser with better animations */
  .teaser {
    display: none;
    align-items: center;
  ...
  .teaser.scale { animation: scaleIn 0.6s cubic-bezier(0.34, 1.56, 0.64, 1); }
  .chatbox.fade { animation: fadeIn 0.6s cubic-bezier(0.4, 0, 0.2, 1); }
  .chatbox.scale { animation: scaleIn 0.6s cubic-bezier(0.34, 1.56, 0.64, 1); }
...
constructor() {
  ...
  // State - keeping original logic intact
...
async connectedCallback() {
  ...
  // Authenticate widget before showing - ORIGINAL LOGIC
  try {
...
// ORIGINAL AUTHENTICATION LOGIC - UNCHANGED
...
// ORIGINAL TOKEN LOGIC - UNCHANGED
...
_initializeWidget() {
  // Auto-show teaser based on config - ORIGINAL LOGIC
...
_toggleChat(open) {
  ...
  // Mobile-optimized focus behavior
  if (open) {
    if (isMobileDevice()) {
      // On mobile, don't auto-focus to prevent keyboard covering widget
      setTimeout(() => this._input.blur(), 50);
    } else {
      // On desktop, focus as normal for better UX
...
async _send() {
  ...
  try {
    ...
    // Mobile-optimized focus behavior after sending
    if (!isMobileDevice()) {
      this._input.focus();
    }
  }
...
// ORIGINAL WIDGET INITIALIZATION - UNCHANGED
...
// Initialize when DOM is ready - ORIGINAL LOGIC
